"""Create TCP load balancer in F5 XC tenant."""
...
def create_lb(event: Dict[str, Any], logger: StructuredLogger) -> Dict[str, Any]:
    """Create a TCP load balancer in F5 XC.
    ...
    """
    step = logger.with_step("create_tcp_lb")
    ...
    step.info("Creating TCP load balancer", name=name, namespace=namespace, domains=spec.get("domains"))

    try:
        client.create_tcp_loadbalancer(namespace, payload)
        step.info("TCP load balancer created", name=name)
        return {"status": "success", "name": name, "created": True}
    except ResourceExistsError:
        step.info("TCP load balancer already exists", name=name)
...
if __name__ == "__main__":
    class MockContext:
        function_name = "tcp_lb_create"

    test_event = {
        "ssm_base_path": "/tenantOps/test",
        "metadata": {"name": "test-mqtt", "namespace": "test"},
        "spec": {"domains": ["test.example.com"], "listen_port": 8883}
    }
