...
def create_origin_pool(_api, namespace: str, origin_name: str, dns_name: str, port: int) -> str:
    """
...
def main(payload: dict):
    ...
    try:
        ...
        namespace = petname
        origin_name_blue = f"{petname}-blue-origin"
        origin_name_green = f"{petname}-green-origin"
        ...
        # Create Origin Pools
        result_message_blue = create_origin_pool(origin_api, namespace, origin_name_blue)
        result_message_green = create_origin_pool(origin_api, namespace, origin_name_green)

        # Wait for Origin Pools to be available
        wait_for_origin_pool(origin_api, namespace, origin_name_blue)
        wait_for_origin_pool(origin_api, namespace, origin_name_green)

        # Create HTTP Load Balancer
        lb_result_message = create_http_load_balancer(lb_api, namespace, lb_name, domain, cert_name, origin_name_blue)

        res = {
            "statusCode": 200,
            "body": f"{result_message_blue} {result_message_green} {lb_result_message}"
        }
